  ...
  methods: {
      ...
      xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
          ...
          _vue.$nextTick(() => {
            ...
            _vue.transformMainTitle(
              Array.from(_vue.$children[0].$el.childNodes)
            );
    ...
    parseURL(data, url) {
      ...
      repo_paths.forEach((path) => {
        if (path.includes(")")) {
          let beforePath =
            repo_paths[repo_paths.findIndex((p) => p == path) - 1];
          if (beforePath != undefined) {
            if (
              beforePath.slice(beforePath.length - 6, beforePath.length) !=
              "github"
            ) {
              ...
              let correspondance = credentials_paths.find(
                (teststring) =>
                  repopath.includes(teststring.author) &&
                  repopath.includes(teststring.repo) &&
              ...
              if (correspondance != undefined && !repopath.includes("pdf")) {
                ...
                data = data.replace(
                  new RegExp(repopath, "g"),
                  (correspondance, decalage) => {
                    if (
                      data.substring(decalage - 2, decalage) == "](" &&
                      data.substring(
                        decalage + correspondance.length,
                        decalage + correspondance.length + 1
                      ) == ")"
                    ) {
                ...
                if (
                  repopath.includes("http") ||
                  repopath.includes("www") ||
                  repopath.slice(0, 1) == "#" ||
                  repopath.includes("pdf")
                ) {
                  ...
                  if (!repopath.includes("..")) {
                    if (repopath.includes(".md")) {
                      let recursiveRepo = this.getContent.url.replace(
                        /readme.md/i,
                        repopath
                      );
                      ...
                      if (this.$route.params.subcontent != undefined) {
                        var newUrl = `https://raw.githubusercontent.com/${author}/${repo}/master/${
                          this.$route.params.subcontent
      ...
      img_url.forEach((string) => {
        if (string.includes('src="')) {
          ...
          if (
            !path[path.length - 1].includes("http") ||
            !path[path.length - 1].includes("www")
          ) {
            if (this.$route.params.subcontent != undefined) {
              var newUrl = `https://raw.githubusercontent.com/${author}/${repo}/master/${
                this.$route.params.subcontent
    ...
    routerLinks(nodesArray) {
      nodesArray.forEach((node) => {
        if (
          Array.from(node.childNodes).length > 0 &&
          ["ul", "li", "ol", "p"].includes(node.localName)
        ) {
          // needed to avoid to much revursioin because of code sections or others unwanted
          this.routerLinks(Array.from(node.childNodes));
        } else {
          if (node.localName == "a") {
            let path_test = this.internalLinks.find(
              (link) => link.path == node.href
            );
              ...
              if (path_test.recursive) {
                node.addEventListener("click", (event) => {
                  ...
                  if (this.$route.params.subcontent) {
                    ...
                    this.$router.push(
                      this.$route.path + "/" + subcontentName.trim()
                    );
                  }
            ...
            if (this.$route.params.subcontent != undefined) {
              if (/..\/README.md/gi.test(node.href)) {
                node.addEventListener("click", (event) => {
            ...
            if (node.href.includes("#")) {
              node.addEventListener("click", (event) => {
                this.$router.replace(
                  this.$route.path + "#" + node.href.split("#")[1]
                );
                event.preventDefault();
              });
            }
    ...
    if (this.getContent.url != undefined) {
      ...
      if (this.$route.params.subcontent != undefined) {
        this.setSubContent({
          ...
          subcontent: this.$route.params.subcontent,
        }).then(() => {
